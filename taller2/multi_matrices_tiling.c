...
__global__ void multi_matricesTiled(int *m1_a, int *m2_a, int *m3_a){
    ...
    for(int m = 0; m < cols / TILE_WIDTH; ++m){
    	m1a_ds[ty][tx] = m1_a[i*cols + m*TILE_WIDTH + tx];
    	m2a_ds[ty][tx] = m2_a[(m*TILE_WIDTH + ty) * cols + j];
    	__syncthreads();

    	for(int k = 0; k < TILE_WIDTH; ++k){
    		aux += m1a_ds[ty][k] * m2a_ds[k][tx];	    
    	}
    	__syncthreads();
    }
    m3_a[i*cols+j] = aux;
